  ...
  <>
    <div  className="relative w-full h-full overflow-hidden ">
      <input
        ...
        data-slot="input"
        className={cn(
          "file:text-foreground placeholder:text-muted-foreground selection:bg-primary !text-[1.1vw] max-md:!text-[2.7vw] max-sm:!text-[3.5vw] max-md:px-[4vw]  selection:text-primary-foreground dark:bg-input/30 border-input flex h-[4vw] max-sm:h-[12vw] max-md:h-[10vw] w-full min-w-0   px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm border border-white/30 rounded-full bg-background inputBlock",
          "",
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen} modal className="!z-[1001]">
      <PopoverTrigger asChild >
        <Button
          type="button"
          variant="outline"
          className="flex justify-center gap-[1.5vw] h-[4vw] border  shadow-none px-[2vw] pb-2  focus:z-900  max-md:h-[10vw] max-sm:h-[12vw] max-sm:w-[35%] max-sm:gap-[4vw] w-[20%] mr-[0.8vw] max-sm:mr-[2.5vw]  bg-[#030815] flagblock  border-white/30 rounded-full cursor-pointer"
          disabled={disabled}
        >
      ...
      <PopoverContent className="w-[300px] p-0 bg-black/10  backdrop-blur-md border border-white/30 rounded-[1.2vw] overflow-hidden">
        <Command>
          <CommandInput
            value={searchValue}
            onValueChange={(val) => {
