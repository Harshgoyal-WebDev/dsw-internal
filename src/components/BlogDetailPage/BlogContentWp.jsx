...
const BlogContentWp = ({ post }) => {
  ...
  useEffect(() => {
    ...
    if (items.length) {
      const intro = items.find((t) => t.title.toLowerCase().includes('introduction'))
  ...
  return (
    <section
      id="content"
      className="h-fit relative max-md:flex-col bg-[#FEFEFE] container flex w-full max-md:!pt-0 pr-[5vw]"
    >
      {/* Info strip */}
      <div className="h-fit absolute max-md:relative max-sm:py-[15vw] max-md:!pt-[0vw] max-md:pb-[10vw] max-md:w-full w-fit blog-info">
        <div className="flex flex-wrap items-center max-md:items-start max-md:justify-between max-md:flex-row-reverse gap-y-[2.5vw] max-md:gap-y-[5vw]">
          <div className="text-[1.05vw] max-sm:text-[4vw] max-md:text-[3vw] space-y-[.8vw] w-[50%] max-md:w-[40%]">
            <Copy><p className="text-background max-md:font-normal font-medium">Publication Date</p></Copy>
            <Copy><p className="text-[#626262]">{formatDate(post.date)}</p></Copy>
          </div>
          <div className="text-[1.05vw] max-sm:text-[4vw] max-md:text-[3vw] space-y-[.8vw] w-[50%] max-md:w-[40%]">
            <Copy><p className="text-background max-md:font-normal font-medium">Category</p></Copy>
            <Copy><p className="text-[#626262]">AI</p></Copy>
          </div>
          <div className="text-[1.05vw] max-sm:text-[4vw] max-md:text-[3vw] space-y-[.8vw] w-[50%] max-md:w-[40%]">
            <Copy><p className="text-background max-md:font-normal font-medium">Reading Time</p></Copy>
            <Copy><p className="text-[#626262]">10 min</p></Copy>
          </div>
          <div className="text-[1.05vw] max-sm:text-[4vw] max-md:text-[3vw] space-y-[.8vw] w-[50%] max-md:w-[40%]">
            <Copy><p className="text-background max-md:font-normal font-medium">Author Name</p></Copy>
            <Copy><p className="text-[#626262]">Dr. Emily Walker</p></Copy>
          </div>
        </div>
      </div>
            ...
            {toc.map((item) => (
              <li
                ...
                className={`text-[1.05vw] max-sm:text-[4vw] max-md:text-[3vw] cursor-pointer transition-all duration-300 hover:text-primary-2 ${
                  activeSection === item.title ? 'text-primary-2' : 'text-background'
                }`}
                title={item.title}
              >
                {item.title}
