...
export default function Build({ allowMultiple = false }) {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-[5vw]">
        <h2 className="text-90 headingAnim text-center capitalize">
          what we build
        </h2>
...
function AccordionItem({
  ...
  return (
    <div
      className={`w-full   overflow-hidden relative faq-tab accordion-block group/parent   ${z} ${index > 0 ? "mt-[-2vw] " : "mt-0"}`}
    >
      <div className={`w-full mr-auto relative`}>
        <div
          className={`inset-0 w-full relative border rounded-[1.5vw] border-white/10 ${isOpen ? "bg-gradient-to-r from-light-blue to-dark-blue" : "bg-[#030815]"}  `}
        >
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-[6vw]">
                  <p className="">{`00${index + 1}`}</p>
                  <h4 className="text-30  text-left leading-[1.2]">{title}</h4>
                </div>

                <div
                  className={`w-[55%]  space-y-4  ${isOpen ? "opacity-100 h-full" : "opacity-0 h-[15vh]"} `}
                >
                  <div
                    className="text-left"
              ...
              <div
                className={` h-auto relative duration-500 max-sm:w-[12vw] rounded-full border-[1.5px] ml-[3vw] p-[2vw]  transition-all  ease-out  ${isOpen ? "border bg-white" : " background-glass border-white/20"}  ${
                  !isOpen
                    ? "group-hover/parent:rotate-[180deg]"
                    : "group-hover/parent:rotate-[315deg] rotate-[45deg]"
                }`}
              >
                <span
              ...
              {isOpen && (
                ...
                >
                  ...
                  <div className="py-[3.5vw] text-[#CACACA] w-[90%] ">
                    {features && (
                      <ul
                        className={`ml-[9vw] list-disc flex flex-wrap gap-x-[3vw] gap-y-[2vw] pb-[5vw]`}
                      >
