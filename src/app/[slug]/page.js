...
//   target:true,
//   btnLink2: '//   btnLink2: 'https://calendly.com/',',
...
export default async function Page({ params }) {
  ...
  // Find post from dummy data
  const post = blogDummyData.find((p) => p.slug === `/${slug}`)

  if (!post) return notFound()

  // Format post to match expected structure
  const formattedPost = {
    ...post,
    categories: post.categories || [{ name: 'AI' }],
    blogFields: {
      relatedBlogs: {
        edges: blogDummyData
          ...
          .map((p) => ({
            node: {
              ...
              featuredImage: {
                node: {
                  sourceUrl: p.featuredImage.sourceUrl,
                },
              },
            },
          })),
  ...
  const pageMeta = getPageMetadata({
    ...
    openGraph: {
      url: `/${slug}`,
      images: [{ url: `${homepage}seo/blog-detail.png`, width: 1200, height: 630 }],
    },
  })
  ...
  return (
    <>
      ...
      <BreadcrumbsJSONLD pathname={`/${slug}`} />
      <Layout>
        <Hero breadcrumbs post={formattedPost}/>
        <BlogContentWp post={formattedPost}/>
        <RelatedArticles post={formattedPost} />
        <FooterCTA footerCTAData={footerCTAData} width="w-[95%]" />
      </Layout>
    </>
  )
