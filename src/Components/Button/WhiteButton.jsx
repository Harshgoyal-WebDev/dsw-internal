...
const WhiteButton = ({ text, background = "", className = "",circleColor, ...props }) => {
  return (
    <Link {...props} data-letters-delay data-split='letters' className={`buttonSplit relative inline-flex h-[4vw] group min-w-[10vw] px-[2vw] items-center gap-3 rounded-full text-[#0A0A0A] group max-sm:h-fit max-sm:py-[5vw] max-sm:px-[6vw] max-sm:gap-[4vw] max-sm:min-w-[30vw] ${className}`}>
