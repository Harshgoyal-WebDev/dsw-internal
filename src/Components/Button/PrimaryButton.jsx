...
const PrimaryButton = ({ text, background = "", className = "", unify, ...props }) => {
  return (
    <Link {...props} data-letters-delay data-split='letters' className={`buttonSplit relative inline-flex items-center h-[4vw] min-w-[10vw] px-[2vw] gap-3 rounded-full overflow-hidden text-white-200 group max-sm:h-fit max-sm:py-[5vw] max-sm:px-[6vw] max-sm:min-w-[30vw] max-sm:gap-[4vw] ${className}`}>
      <span className={`bg-foreground rounded-full h-2 w-2 max-sm:w-[2.5vw] max-sm:h-[2.5vw] z-[1] ${unify?"!bg-black":""}`}></span>
